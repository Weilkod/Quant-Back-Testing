"""
╔══════════════════════════════════════════════════════════════════════╗
║  Quant-Alpha v3.4 — 통합 알고리즘 (Unified Single-File)             ║
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  통합 대상 (6개 모듈):                                                ║
║    § 1. config.py           하이퍼파라미터 중앙 관리 (45개 상수)       ║
║    § 2. countries.py        5개국 설정 + 금리 alpha + 거래비용         ║
║    § 3. macro_engine.py     VIX 그라데이션 + 매크로 스코어 + 국면분류   ║
║    § 4. algorithm.py        Gate + Score + Stop + Action + Pipeline   ║
║    § 5. position.py         개별 포지션 사이징                         ║
║    § 6. portfolio_manager.py 포트폴리오 리스크 관리 (5개 제약)          ║
║    § 7. test_structure.py   구조 검증 스크립트                         ║
║                                                                      ║
║  파이프라인 (9단계):                                                  ║
║    ① 데이터 수집 → ② Survival Gate (5개) → ③ Z-Score+매크로 가중치    ║
║    → ④ 통합 점수 (10팩터, 100점) → ⑤ 트레일링 스탑 → ⑥ 실행 판단     ║
║    → ⑦ 포지션 사이징 → ⑧ 포트폴리오 리스크 관리 → ⑨ 비용 필터        ║
║                                                                      ║
║  변경 이력: v3.2 → v3.3 (Patch 1~3) → v3.4 (항목 1~7 + 이슈 10건)   ║
║  Patch 1: VIX 4단계 그라데이션                                        ║
║  Patch 2: 컨센서스 결측치 동적 재분배                                  ║
║  Patch 3: TREND_HOLD 액션 도입                                        ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
SCORE_BUY_THRESHOLD = 85                  # L3  S_BUY / TREND_BUY / TREND_HOLD 진입
SCORE_HOLD_THRESHOLD = 70                 # L3  HOLD vs REDUCE 경계
...
PATCH_HISTORY = [
    ...
    "Patch 4: v3.4 → v3.4.1 phase1 통합 마이그레이션 (unified → phase1 분리)",
]
...
def evaluate_survival_gate(
    ...
    if metrics.ocf is not None and metrics.ocf <= 0:
        details["gate_3_cashflow"] = "FAIL"
        fail_code = fail_code or "FAIL_OCF"
...
def test_algorithm_score():
    ...
    print(f"     결측(consensus+SI) 스코어: {score_missing:.1f}점")

    print("  ✅ algorithm 스코어 검증 통과")
