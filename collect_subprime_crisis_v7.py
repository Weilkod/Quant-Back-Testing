...
"""
=============================================================================
  시나리오 2: 서브프라임 모기지 위기 백테스트용 데이터 수집 스크립트
  문서번호: QA-2026-BT-SUBPRIME
=============================================================================
  ...
  시나리오 설명:
    기준 시점: 서브프라임 위기 정점 2008년 9월 (리먼 브라더스 파산)
    수집 기간: 2002-01-01 ~ 2014-12-31 (전 6년 + 후 6년)
    종목 기준: 2002년 S&P 500 시가총액 상위 50개
    목적: 생존자 편향(Survivorship Bias) 제거

  ⚠️ 핵심 관찰 포인트:
    - 2002년 기준 금융섹터 10종목(20%) → 2008년 대거 파산/구제금융
    - Citigroup($55→$1), AIG(99% 폭락), Fannie Mae/Freddie Mac(상폐)
    - Wachovia(뱅크런 후 WFC에 인수)
    - 이들의 "폭락→상폐" 수익률이 포함되어야 현실적 백테스트 가능
    - 현재 시점 종목으로 백테스트하면 이 손실이 전부 누락됨

  변경 이력:
    v3.0 - collect_backtest_data_v3.py 기반 생성
         - 2002년 기준 S&P 500 Top 50 종목 적용
         - 합병/상폐/구제금융 종목 처리 로직 추가
         - validate() 함수 금융위기 종목 예외 처리

  실행:
    pip install requests yfinance pandas
    python collect_subprime_crisis.py
=============================================================================
"""
...
TICKERS_PARTIAL_DATA = {
    "FNM": "2010-06-16",   # Fannie Mae → 상폐 (OTC 이동)
    "FRE": "2010-07-07",   # Freddie Mac → 상폐 (OTC 이동)
    "WB":  "2008-12-31",   # Wachovia → Wells Fargo 인수
}

# yfinance에서 조회 시 사용할 대체 티커
# (일부 상폐 종목은 OTC 티커로 조회 가능할 수 있음)
TICKER_ALT = {
    "FNM":  "FNMA",    # OTC Pink Sheets
    "FRE":  "FMCC",    # OTC Pink Sheets
    "WB":   None,      # 완전 소멸
    "WAG":  "WBA",     # 2014 Walgreens Boots Alliance
}

# 2008 금융위기 직격탄 종목 (참고용 메타데이터)
CRISIS_IMPACT = {
    "C":   "Citigroup    — 주가 $55→$1, 구제금융 $450억, 1:10 역분할",
    "AIG": "AIG          — 주가 $70→$0.35, 구제금융 $1,850억",
    "BAC": "BofA         — Merrill Lynch 인수, 구제금융 $450억",
    "FNM": "Fannie Mae   — 정부 인수(conservatorship), 상폐",
    "FRE": "Freddie Mac  — 정부 인수(conservatorship), 상폐",
    "WB":  "Wachovia     — 뱅크런 후 Wells Fargo에 헐값 인수",
    "GE":  "GE           — GE Capital 부실로 주가 $40→$6",
}
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("collect_subprime_log.txt", encoding="utf-8"),
    ],
)
...
def generate_universe():
    ...
    filepath = UNIV_DIR / "sp500_2002.csv"
...
def get_alt_ticker(ticker):
    """상폐 종목의 대체 티커 반환 (OTC 등)"""
    return TICKER_ALT.get(ticker)
...
def fetch_prices():
    ...
    log.info("      ⚠️  합병/상폐/구제금융 종목 포함 (중도 종료 정상 처리)")
    log.info("=" * 60)
    ...
    for ticker in TICKERS:
        ...
        crisis_flag = ""
        if ticker in CRISIS_IMPACT:
            crisis_flag = " [🔥 금융위기 직격]"
        elif is_partial_ticker(ticker):
            crisis_flag = " [합병/상폐]"

        # --- FMP 메인 ---
        log.info(f"  [{TICKERS.index(ticker)+1}/{TOTAL_TICKERS}] {ticker}{crisis_flag} ─ FMP 시도...")
        ...
        yf_ticker = ticker
        try:
            df = yf.download(yf_ticker, start=START_DATE, end=ticker_end,
                             auto_adjust=False, progress=False)

            # 원본 티커 실패 시 대체 티커 시도
            if df.empty and get_alt_ticker(ticker):
                alt = get_alt_ticker(ticker)
                log.info(f"    ⚠️  {ticker} 없음 → 대체 티커 {alt} 시도...")
                df = yf.download(alt, start=START_DATE, end=ticker_end,
                                 auto_adjust=False, progress=False)
    ...
    if fail > 0:
        log.warning(f"  ⚠️  실패 {fail}개 중 합병/상폐 종목은 데이터 소스에서 미지원 가능")
...
def fetch_fundamentals():
    ...
    log.info("      ⚠️  2002~2007 구간은 XBRL 미적용으로 데이터 부재 가능 (수용)")
    log.info("=" * 60)
    ...
    for i, ticker in enumerate(TICKERS, 1):
        ...
        if ticker in CRISIS_IMPACT:
            ...
            crisis_flag = " [합병/상폐]"
        log.info(f"  [{i}/{TOTAL_TICKERS}] {ticker}{crisis_flag}")
...
# ============================================================================
# 8. [5/7] 컨센서스 추정치: FMP analyst-estimates
# ============================================================================
...
def fetch_consensus():
    ...
    for i, ticker in enumerate(TICKERS, 1):
        ...
        log.info(f"  [{i}/{TOTAL_TICKERS}] {ticker}")
...
# ============================================================================
# 9. [6/7] 어닝서프라이즈: FMP earnings-calendar (연도별 수집 → 종목 필터)
# ============================================================================
...
def fetch_earnings_surprise():
    ...
    log.info("      ⚠️  2002~2007 구간은 데이터 부재 가능")
    log.info("=" * 60)
...
def validate():
    ...
    min_expected = TOTAL_TICKERS - partial_count
    ...
    log.info(f"\n  ── 🔥 금융위기 직격탄 종목 현황 ──")
    for ticker, desc in CRISIS_IMPACT.items():
        pfile = PRICES_DIR / f"{ticker}.csv"
        if pfile.exists():
            rows = sum(1 for _ in open(pfile)) - 1
            log.info(f"  🔥 {ticker}: {rows}행 — {desc.split('—')[1].strip()}")
        else:
            log.warning(f"  ⚠️  {ticker}: 파일 없음")
...
def main():
    ...
    log.info("🚀 서브프라임 모기지 위기 시나리오 데이터 수집 스크립트")
    log.info(f"   시나리오: 서브프라임 위기 (정점 2008.09, 리먼 파산)")
    log.info(f"   전략: FMP Premium 메인 → yfinance/FRED 폴백")
    log.info(f"   수집 기간: {START_DATE} ~ {END_DATE}")
    log.info(f"   종목 기준: 2002년 S&P 500 시가총액 상위 {TOTAL_TICKERS}개")
    log.info(f"   합병/상폐 종목: {sum(1 for t in TICKERS if is_partial_ticker(t))}개")
    log.info(f"   금융위기 직격탄: {len(CRISIS_IMPACT)}개")
